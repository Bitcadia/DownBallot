...
import { zip, takeWhile, times } from "lodash";
...
async function readOrFetch(url: string, folder: string, split: string) {
    ...
    try {
        return await readFile(`${folder}/${url.split(split)[1].replace(/[:]/g, "")}`, { encoding: "utf-8" });
    }
    catch {
        ...
        running--;
        await writeFile(`${folder}/${url.split(split)[1].replace(/[:]/g, "")}`, JSON.stringify(text), "utf-8");
...
interface Result {
    ...
    "locality_fips": string,
    "is_geographic": boolean,
    "votes": number
}
export const Results = (async () => {
    const split = "precincts/";
    const roots = await Promise.all(Urls.map(async (url) => {
        return { root: JSON.parse(await readOrFetch(url, "Results", split)) as { precincts: Result[] }, url };
    }));
    return roots.reduce((acc, curr, index) => {
        let [_name, timeStamp] = curr.url.split("GAGeneral-");
        timeStamp = timeStamp.slice(0, -5);
        const votes = curr.root.precincts.reduce((acc, result) => {
            acc[`${result.locality_name}-${result.precinct_id}`] = (acc[`${result.locality_name}-${result.precinct_id}`] || 0) + result.votes;
            return acc;
        }, {} as { [precinct: string]: number });
        Object.keys(votes).reduce((accumulator, current) => {
            accumulator[current] = (accumulator[current] || []);
            const delta = votes[current] - ((accumulator[current]).slice(-1)[0]?.total || 0);
            delta && accumulator[current].push({ total: votes[current], delta, precinct: current, timeStamp })
            return accumulator;
        }, acc)
        return acc;
    }, {} as { [precinct: string]: { total: number, delta: number, precinct: string, timeStamp:string }[] });
});
